...
const indexJWSecret = require("./index");
        ...
        try {
            ...
            self.db.get(`select *  from user where username = ? AND password = ?`, [req.body.username, md5(req.body.password)], (err, row) => {
                ...
                if (row.count === 0) {
                    ...
                    const token = jwt.sign({ id: row.id, username: row.username }, indexJWSecret.JWT_SECRET_TOKEN);
                    res.cookie("access_token", token, {}).status(200).json({ message: "Successfully logged in" });
                }
            })
